...
internal static class Gen5ComputeFixtures
{
    ...
    // End-to-end executable program: real ALU results stored to guest_buffer0 at dword
    // offsets 0/4/8, then proof that a store with EXEC=0 does not land (offset 12 keeps
    // its sentinel) and that stores work again once EXEC is restored (offset 16).
    ...
    public static readonly Gen5ComputeFixture[] All = [Fmac, Muls, ExecStore];
    ...
    public static Gen5MslShader CompileOrThrow(Gen5ComputeFixture fixture)
    {
        ...
        // Buffer stores need a global-memory binding; the emitters resolve them by
        ...
        var globalBindings = storePcs.Length != 0
            ? new[]
            {
                new Gen5GlobalMemoryBinding(
                    ...
                    0UL,
                    storePcs,
                    new byte[fixture.StoreBackingBytes]),
            }
